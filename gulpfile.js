...
//                                copy site files, index with Pagefind
...
//       site/                ← HTML pages, scripts, and site images
//       images/              ← Source images (copied from src/images)
//       _pagefind/           ← Search index (generated by Pagefind)
//       index.html           ← Homepage
//
// ============================================================================
...
/**
 * Copy site HTML files to dist
 *
 * Copies index.html and the site/ directory (HTML pages, scripts, images)
 * so the full site is available in the deploy output. Also copies src/images
 * to dist/images for any images referenced directly from HTML.
 */
async function copySiteFiles() {
  ...
  if (fs.existsSync('./index.html')) {
    fs.cpSync('./index.html', path.join(distFolderLocation, 'index.html'));
    log(colors.blue, `    index.html → ${distFolderLocation}/index.html`);
  }

  // Copy site/ directory (HTML pages, scripts, etc.)
  if (fs.existsSync('./site')) {
    fs.cpSync('./site', path.join(distFolderLocation, 'site'), { recursive: true });
    log(colors.blue, `    site/ → ${distFolderLocation}/site/`);
  }

  // Copy src/images to dist/images (for any images referenced from HTML)
  if (fs.existsSync('./src/images')) {
    fs.cpSync('./src/images', path.join(distFolderLocation, 'images'), { recursive: true });
    log(colors.blue, `    src/images/ → ${distFolderLocation}/images/`);
  }
}

/**
 * Index the site with Pagefind for search functionality
 *
 * Runs the Pagefind CLI to crawl all HTML in dist/ and generate a static
 * search index at dist/_pagefind/. This index is loaded by the search
 * results page to provide client-side full-text search.
 */
async function indexWithPagefind() {
  log(colors.blue, `\nIndexing site with Pagefind...`);
  execSync('npx pagefind --site dist --output-path dist/_pagefind', { stdio: 'inherit' });
  log(colors.blue, `Pagefind indexing complete.`);
}

/**
 * Main build task: Clean → Compile Sass → Copy all assets → Copy site files → Index with Pagefind
...
async function doBuildAll () {
  ...
  series(doClean, doSassCompile, buildDist, copySiteFiles, indexWithPagefind)()
}
...
// │                                                                         │
// │   npm run build          → Full build (clean + compile + copy + search) │
// │   npm run build:styles   → Compile Sass only                            │
// │   npm run watch:styles   → Watch Sass files and recompile on changes    │
// │   npm run clean          → Remove dist/ folder                          │
// │                                                                         │
